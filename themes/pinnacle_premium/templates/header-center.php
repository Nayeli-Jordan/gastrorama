<?php global $pinnacle, $post; 
  ...
  if(isset($pinnacle['logo_container_width'])) {
      if($pinnacle['logo_container_width'] == '50') {
                ...
                $logocclass = 'kt-logo-25 col-md-4 col-sm-8 col-ss-9'; 
                $menulclass = 'kt-menu-left kt-logo-25 col-md-4 col-sm-4 col-ss-3'; 
                $menurclass = 'kt-menu-right kt-logo-25 col-md-4 col-sm-4 col-ss-3';
      } else {
                $logocclass = 'col-md-2 col-sm-8 col-ss-9'; 
                $menulclass = 'kt-menu-left col-md-5 col-sm-4 col-ss-3'; 
                $menurclass = 'kt-menu-right col-md-5 col-sm-4 col-ss-3';
      }
  } else {
    $logocclass = 'col-md-4 col-sm-8 col-ss-9'; 
    $menulclass = 'kt-menu-left col-md-4 col-sm-4 col-ss-3'; 
    $menurclass = 'kt-menu-right col-md-4 col-sm-4 col-ss-3';
  }

  ?>
<header id="kad-banner" class="banner headerclass kad-header-style-basic kad-header-center-logo <?php echo esc_attr($mstickyclass);?>" data-header-shrink="<?php echo esc_attr($shrinkh); ?>" data-mobile-sticky="<?php echo esc_attr($msticky);?>" data-pageheaderbg="<?php echo esc_attr($pageheaderbg);?>" data-header-base-height="<?php echo esc_attr($header_height);?>">
<?php if (kadence_display_topbar()) : 
      get_template_part('templates/header', 'topbar'); 
      endif; 
      do_action('kt_before_header_content'); ?>

          <style type="text/css"> .kad-header-style-basic .kad-primary-nav ul.sf-menu > li > a, .kad-header-style-basic #kad-shrinkheader #mobile-nav-trigger a {line-height:<?php echo esc_attr($header_height);?>px; }  </style>
  <div id="kad-shrinkheader" class="container" style="height:<?php echo esc_attr($header_height);?>px; line-height:<?php echo esc_attr($header_height);?>px;">
    <div class="row">
    <div class="<?php echo esc_attr($menulclass); ?> kad-header-menu-left">
          ...
          <div class="<?php echo esc_attr($logocclass); ?> clearfix kad-header-left kad-header-logo">
            <div id="logo" class="logocase">
              <a class="brand logofont" style="height:<?php echo esc_attr($header_height);?>px; line-height:<?php echo esc_attr($header_height);?>px; display:block;" href="<?php echo home_url(); ?>/">
                      <?php if (!empty($pinnacle['x1_logo_upload']['url'])) { ?> 
                       ...
                       <div style="background-image: url('<?php echo $pinnacle['x1_logo_upload']['url']; ?>'); max-height:<?php echo esc_attr($header_height);?>px; height:<?php echo esc_attr($pinnacle['x1_logo_upload']['height']); ?>px; width:<?php echo esc_attr($pinnacle['x1_logo_upload']['width']); ?>px;" class="kad-standard-logo kad-lg" /></div>
                       <?php if (!empty($pinnacle['th_x1_logo_upload']['url'])) { ?> 
                       <div style="background-image: url('<?php echo esc_url($pinnacle['th_x1_logo_upload']['url']); ?>'); max-height:<?php echo esc_attr($header_height);?>px; height:<?php echo esc_attr($pinnacle['th_x1_logo_upload']['height']); ?>px; width:<?php echo esc_attr($pinnacle['th_x1_logo_upload']['width']); ?>px;" class="kad-lg kad-standard-logo-tranbg" /></div> <?php }?>
                        </div> <?php } else { echo apply_filters('kad_site_name', get_bloginfo('name')); } ?>
              </a>
           </div> <!-- Close #logo -->
       </div><!-- close col -->
       <div class="<?php echo esc_attr($menurclass); ?> kad-header-right">
              ...
              <div class="kad-nav-inner mobileclass">
                <div id="mobile_menu_collapse" class="kad-nav-collapse collapse mobile_menu_collapse">
                   <?php if(isset($pinnacle['menu_search']) && $pinnacle['menu_search'] == '1') {
                            if(isset($pinnacle['menu_search_products']) && $pinnacle['menu_search_products'] == '1' && class_exists('woocommerce') ) {
