    ...
    <li class="clearfix postclass kt-top-featured">
        ...
        <a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>" class="recentpost_featimg">
             <?php if(has_post_thumbnail( $post->ID ) ) { 
                ...
                echo '<img width="60" height="60" src="'.esc_attr($image).'" class="attachment-widget-thumb wp-post-image" alt="">'; 
              } 
          }?>
        </a>
        <a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>" class="recentpost_title">
          <?php the_title(); ?>
        </a>
        <span class="recentpost_date color_gray"><?php echo get_the_date(get_option( 'date_format' )); ?></span>
          <?php  if($feature == "true" && $i == 1) { ?>
           <span class="postcontent">
           <?php if(function_exists('the_advanced_excerpt') ) {
                                    the_excerpt();
                                    } else { ?>
                                  <p><?php echo pinnacle_excerpt(20); ?> <a href="<?php the_permalink() ?>"><?php global $pinnacle; if(!empty($pinnacle['post_readmore_text'])) {$readmore = $pinnacle['post_readmore_text'];} else {$readmore = __('Read More', 'virtue');} echo $readmore; ?></a></p>
                                  <?php } ?> 
           </span>
           <?php } ?>
      </li>
...
<?php echo $after_widget; 
  ...
  public function form( $instance ) {
    ...
    $featureoptions = array(array('name' => 'False', 'slug' => 'false'), array('name' => 'True', 'slug' => 'true'));
    ...
    foreach ($featureoptions as $foption) {
...
?>
   ...
   <p>
    <label for="<?php echo $this->get_field_id('first_feature'); ?>"><?php _e('Feature first item:', 'pinnacle'); ?></label>
              ...
              <div class="grid_item testimonial_item all">
                ...
                <?php global $post; ?>
                  ...
                  <?php if(isset($link) && $link == 'page') {
                                    ...
                                    if(!empty($linktext)) $thelinktext = $linktext; else {$thelinktext = __('Read More', 'pinnacle');}
                                    echo $thelinktext;
                                    echo '</a>';
...
<?php
  ...
  public function form( $instance ) {
    ...
    $linkoptions = array(array('name' => __('none', 'pinnacle'), 'slug' => 'false'), array('name' => __('Page Link', 'pinnacle'), 'slug' => 'page'), array('name' => __('Post Link', 'pinnacle'), 'slug' => 'post'));
...
?>
    ...
    <p>
    <label for="<?php echo $this->get_field_id('link'); ?>"><?php _e('Link Options:', 'pinnacle'); ?></label>
    <select id="<?php echo $this->get_field_id('link'); ?>" name="<?php echo $this->get_field_name('link'); ?>"><?php echo implode('', $link_options); ?></select>
    </p>
    <p>
    <label for="<?php echo $this->get_field_id('pagelink'); ?>"><?php _e('If link to page, choose page:', 'pinnacle'); ?></label>
    <select id="<?php echo $this->get_field_id('pagelink'); ?>" name="<?php echo $this->get_field_name('pagelink'); ?>"><?php echo implode('', $pagelink_options); ?></select>
     ...
     <p><label for="<?php echo $this->get_field_id('linktext'); ?>"><?php _e('Link text (e.g. = Read More)', 'pinnacle'); ?></label>
