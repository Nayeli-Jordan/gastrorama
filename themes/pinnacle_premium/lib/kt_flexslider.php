            ...
            foreach ($attachments as $attachment) {
              $attachment_post = get_post( $attachment  );
                ...
                if(empty($image)) {$image = $attachment_url;} ?>
                    <li>
                        <a <?php if($link == "post") { echo 'href="'.get_the_permalink().'" title="'.get_the_title().'"'; } else { echo 'href="'.esc_url($attachment_url).'" data-rel="lightbox"'; } ?>>
